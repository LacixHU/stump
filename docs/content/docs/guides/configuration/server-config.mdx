...
<Callout type="info">
	Stump will use default options if you don't manually configure them. When following this guide,
...
The OP address that the Stump server listens on.
...
See the [Database](/docs/guides/configuration/database) guide for more information on configuring the database.
...
The database connection URL. Technically, this will work for either PostgreSQL or SQLite, but [STUMP_DB_PATH](#stump_db_path) is the preferred way to configure SQLite.

| Type   | Default Value | TOML Key                  |
| ------ | ------------- | ------------------------- |
| String |               | Not configurable via TOML |

### STUMP_DB_HOST

The host of the database. This is only used when connecting to a PostgreSQL database, and not required if [STUMP_DATABASE_URL](#stump_database_url) is set.
...
The port of the database. This is only used when connecting to a PostgreSQL database, and not required if [STUMP_DATABASE_URL](#stump_database_url) is set.

| Type    | Default Value | TOML Key                  |
| ------- | ------------- | ------------------------- |
| Integer |               | Not configurable via TOML |

### STUMP_DB_USER

The username for the database. This is only used when connecting to a PostgreSQL database, and not required if [STUMP_DATABASE_URL](#stump_database_url) is set.
...
The password for the database. This is only used when connecting to a PostgreSQL database, and not required if [STUMP_DATABASE_URL](#stump_database_url) is set.
...
The name of the database. This is only used when connecting to a PostgreSQL database, and not required if [STUMP_DATABASE_URL](#stump_database_url) is set.

| Type   | Default Value | TOML Key                  |
| ------ | ------------- | ------------------------- |
| String |               | Not configurable via TOML |

### STUMP_DB_TIMEOUT_SECS

The timeout for both SQLite and PostgreSQL database connections, in seconds. It behaves differently for each database:

- For SQLite, this is the timeout used for both acquiring and busy waiting for a database connection
- For PostgreSQL, this is the timeout used for acquiring a database connection
...
Directory for user-supplied emulator firmware/BIOS (for example Amiga Kickstart ROMs). Files are never bundled with Stump. Defaults to `{config_dir}/firmware` when unset. Served via authenticated `GET /api/v2/firmware/{file_name}` (basename only).

| Type   | Default Value           | TOML Key       |
| ------ | ----------------------- | -------------- |
| String | `{config_dir}/firmware` | `firmware_dir` |

See [Retro computer libraries](/docs/guides/features/retro-libraries).
