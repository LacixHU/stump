...
There are way too many varying organizational preferences for Stump to support all of them. Instead, Stump supports three different patterns for organizing your library:
...
- **Nested**: Builds a parent/child series tree from the full folder hierarchy (e.g. Author → Series → books).
...
A nested library turns the folder tree into a **series hierarchy**. Every directory that directly contains media becomes a series, and every ancestor folder up to the library root also becomes a series (so intermediate folders without loose files still appear as parents).

- Media always belong to the series whose folder **directly** contains the file.
- Parent series can have their own books and child series (for example, an author folder with a standalone ebook plus a Mistborn subfolder).
- The library series view lists **root** series first; open a series to browse sub-series and breadcrumbs.
...
<Callout>This section is not feature complete and is subject to change in the future.</Callout>
...
You can explicitly include users who may access a library. Users who are not included cannot see the library or its contents. The server owner always has access and does not need to be included.
