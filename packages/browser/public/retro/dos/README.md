...
`wdosbox.js` is patched so the emscripten module exposes `SDL` and `Asyncify` (`Module["SDL"]=SDL;Module["Asyncify"]=Asyncify` before `return WDOSBOX`). Save/load needs those to pause audio and reset the main loop. Re-apply that line after upgrading.
