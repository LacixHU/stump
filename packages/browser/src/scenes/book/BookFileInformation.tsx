		...
		<div className="space-y-1 pb-3 pt-2 text-sm flex flex-col">
			...
			<div className="space-x-4 flex">
				<Text size="sm" variant="muted">
					{t(getKey('labels.fileSize'))}: {formatBytes(data.size)}
				</Text>
				<Text size="sm" variant="muted">
					{t(getKey('labels.fileType'))}: {data.extension?.toUpperCase()}
				</Text>
			</div>
			{data.hash && (
				<Text size="sm" variant="muted" title={data.hash || ''}>
					{t(getKey('labels.fileChecksum'))}: {formatHash(data.hash || '')}
				</Text>
			)}
			<Text size="sm" variant="muted" title={data.relativeLibraryPath}>
				{t(getKey('labels.fileLocation'))}: {data.relativeLibraryPath}
			</Text>
		</div>
	)
}

const LOCALE_KEY = 'bookOverviewScene.fileInformation'
