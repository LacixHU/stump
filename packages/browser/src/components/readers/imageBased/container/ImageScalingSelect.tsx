...
export default function ImageScalingSelect({ value, onChange }: Props) {
	...
	return (
		<div className="py-1.5">
			<Label htmlFor="image-scaling-fit">{t('reader.imageScaling')}</Label>
			<NativeSelect
				id="image-scaling-fit"
				size="sm"
				options={[
					{ label: t('reader.options.auto'), value: 'AUTO' },
					{ label: t('reader.options.height'), value: 'HEIGHT' },
					{ label: t('reader.options.width'), value: 'WIDTH' },
					{ label: t('reader.options.original'), value: 'NONE' },
				]}
				value={value}
