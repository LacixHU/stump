...
export default function ReadingModeSelect({ value, onChange }: Props) {
	...
	return (
		<div className="py-1.5">
			<Label htmlFor="reading-mode">{t('reader.flow')}</Label>
			<NativeSelect
				id="reading-mode"
				size="sm"
				options={[
					{ label: t('reader.options.verticalScroll'), value: 'CONTINUOUS_VERTICAL' },
					{ label: t('reader.options.horizontalScroll'), value: 'CONTINUOUS_HORIZONTAL' },
					{ label: t('reader.options.paged'), value: 'PAGED' },
				]}
				value={value}
